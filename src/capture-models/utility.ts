import { CaptureModel, FieldMapItem, Templates, FieldMap, AllStructures } from './types';

export function generateBlankField<T extends Templates, F extends FieldMap<keyof T>>(
	captureModel: CaptureModel<T, F>,
	field: FieldMapItem<keyof T>
) {
	// Grab the last field.
	...
	if (last.type === 'model') {
		const fields: typeof last.fields = Object.keys(
			last.fields
		).reduce(<T extends keyof typeof last.fields>(acc: any, key: T) => {
